...
public class NaveDestructora extends VehiculoGuerra implements Tripulable {
	...
	@Override
	public int atacar() {
		int ataqueTotal = (int) (this.getAtaque() * Math.random()); //ATAQUE DEL VEHICULO * RANDOM (0,1)
	...
	public int defender(int ataqueRecibido) {
		int defensaTotal = (int) (this.getDefensa() * Math.random()*0.1);
		...
		destruccion = this.getPuntosVida() - destruccion;
		this.setPuntosVida(destruccion);
		
		System.out.println(this.getNombre() + " se defiende con " + defensaTotal + " puntos, le quedan " + destruccion + " puntos de vida");
		return defensaTotal;
	}
}
